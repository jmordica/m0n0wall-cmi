     ...
     ?><ul><?php
      if (($l = mysql_connect($_POST["dbhostname"], $_POST["username"], $_POST["password"])) === FALSE) {
        ...
        if (mysql_select_db($_POST["dbname"], $l) === FALSE) {
          ...
          ?><li><span class="greencolor">OK</span>: Database <?=$_POST["dbname"];?> selected...</li><?php
          /* create tables */
          $sqlf = glob("./sql/*.sql");   
 	  if (count($sqlf)) {
	    foreach ($sqlf as $sqlfile) {
	      ...
	      if (mysql_exec_batch($sqlq, $l) === FALSE) {
	         ?><li><span class="redcolor">ERROR</span>: Cannot create <?=$table;?></li><?php
	         $error++;
	      } else {
		 ?><li><span class="greencolor">OK</span>: Created table <?=$table;?>...</li><?php
	      }
 	    }
	  }
