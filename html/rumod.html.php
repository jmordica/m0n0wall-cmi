...
//   $rpos = count($ifa->rulesint);
   $rpos->position = $ifa->rulesint[count($ifa->rulesint) - 1]->position + 1;
  break;
  case 1: /* edit rule */
   $action = 4;
   if (isset($rpos)) {
  ...
  <td width="78%" class="vtable">
   ...
   <?php $interfaces = array('wan' => 'WAN', 'lan' => 'LAN', 'pptp' => 'PPTP');
         ...
         foreach ($interfaces as $iface => $ifacename): ?>
           <option value="<?=$iface;?>" <?php if ($iface == $ifn) echo "selected"; ?>>
 ...
 <tr> 
  ...
  <td width="78%" class="vtable">
   <select name="proto" class="formfld" onchange="proto_change()">
     ...
     <option value="<?=strtolower($proto);?>" <?php if (strtolower($proto) == strtolower($rule->protocol)) echo "selected"; ?>>
...
<?php
                      ...
                      <option value="<?=$icmptype;?>" <?php if ($icmptype == $rule->icmptype) echo "selected"; ?>>
                ...
                <tr> 
                  ...
                  <td width="78%"> 
                    <input name="Submit" type="submit" class="formbtn" value="Save"> 
                                        ...
                                        <input name="iid" type="hidden" value="<?=$ifa->id;?>"> 
                                        <input name="rid" type="hidden" value="<?=$rule->id;?>"> 
<?php if ($action == 3) { ?>		<input name="rpos" type="hidden" value="<?=$rpos;?>"> <?php } ?>
...
?>
  ...
  <a href="viewfw.php?mid=<?=$mid;?>iid=<?=$iid;?>">Return to firewall view</a>
  <?php
  } else {
  ?>
...
<?php
  ...
  ?>
  <a href="viewfw.php?mid=<?=$mono->id;?>&iid=<?=$ifa->id;?>">Return to FW View</a>
<?php } else { ?>
