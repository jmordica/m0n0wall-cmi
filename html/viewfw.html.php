...
<tr valign="top" id="fr<?=$nrules;?>">
 <td class="listt"><input type="checkbox" id="frc<?=$nrules;?>" name="rule[]" value="<?=$ru->rule->id;?>" onClick="fr_bgcolor('<?=$nrules;?>')" style="margin: 0; padding: 0; width: 15px; height: 15px;"></td>
...
?>
  <a href="rumod.php?action=7&mid=<?=$mono->id;?>&iid=<?=$ifa->id; ?>&rpos=<?php echo $ru->position; ?>"><img src="img/<?=$iconfn;?>.gif" width="11" height="11" border="0" title="click to toggle enabled/disabled status"></a>
<?php if ($ru->rule->log):
        $iconfn = "log_s";
        if (!$ru->enabled)
          $iconfn .= "_d";
?>
  ...
  <td class="listlr" onClick="fr_toggle(<?=$nrules;?>)"> 
    <?=$textss;?><?php if (!empty($ru->rule->protocol)) echo strtoupper($ru->rule->protocol); else echo "*"; ?><?=$textse;?>
  </td>
  <td class="listr" onClick="fr_toggle(<?=$nrules;?>)">
   <?=$textss;?><?php if(!empty($ru->rule->source)) echo htmlspecialchars($ru->rule->source); else echo "*"; ?><?=$textse;?>
  </td>
  <td class="listr" onClick="fr_toggle(<?=$nrules;?>)">
   <?=$textss;?><?php if(!empty($ru->rule->sport)) echo htmlspecialchars($ru->rule->sport); else echo "*"; ?><?=$textse;?>
  </td>
  <td class="listr" onClick="fr_toggle(<?=$nrules;?>)"> 
   <?=$textss;?><?php if(!empty($ru->rule->destination)) echo htmlspecialchars($ru->rule->destination); else echo "*"; ?><?=$textse;?>
  </td>
  <td class="listr" onClick="fr_toggle(<?=$nrules;?>)"> 
   <?=$textss;?><?php if(!empty($ru->rule->dport)) echo htmlspecialchars($ru->rule->dport); else echo "*"; ?><?=$textse;?>
  </td>
  <td class="listbg" onClick="fr_toggle(<?=$nrules;?>)"> 
   <?=$textss;?><?=htmlspecialchars($ru->rule->description);?>&nbsp;<?=$textse;?>
  </td>
  <td valign="middle" nowrap class="list">
   <table border="0" cellspacing="0" cellpadding="1">
    <tr>
     <td><input name="move[<?=$ru->position;?>]" value="<?=$ru->position;?>" type="image" src="img/left.gif" width="17" height="17" title="move selected rules before this rule" onMouseOver="fr_insline(<?=$nrules;?>, true)" onMouseOut="fr_insline(<?=$nrules;?>, false)"></td>
    ...
    <tr>
     ...
     <td><a href="rumod.php?action=2&rid=<?=$ru->rule->id;?>&rpos=<?php echo $ru->position;?>&iid=<?php echo $ifa->id; ?>&mid=<?php echo $mono->id; ?>"><img src="img/plus.gif" title="add a new rule based on this one" width="17" height="17" border="0"></a></td>
    </tr>
   </table>
  </td>
 </tr>
...
Click the <a href="rumod.php?mid=<?=$mono->id?>&iid=<?=$ifa->id;?>"><img src="img/plus.gif" title="add new rule" border="0" width="17" height="17" align="absmiddle"></a> button to add a new rule.</span>
</td>
<?php } ?>
...
<tr id="fr<?=$nrules;?>"> 
 ...
 <td class="list">
  <table border="0" cellspacing="0" cellpadding="1">
   <tr>
    <td>
     <?php if ($nrules == 0): ?><img src="img/left_d.gif" width="17" height="17" title="move selected rules to end" border="0"><?php else: ?><input name="move" value="<?=$i+1;?>" type="image" src="img/left.gif" width="17" height="17" title="move selected rules to end" onMouseOver="fr_insline(<?=$nrules+1;?>, true)" onMouseOut="fr_insline(<?=$nrules+1;?>, false)"><?php endif; ?></td>
    <td></td>
   </tr>
   <tr>
    ...
    <td><a href="rumod.php?mid=<?=$mono->id;?>&iid=<?=$ifa->id;?>"><img src="img/plus.gif" title="add new rule" width="17" height="17" border="0"></a></td>
   </tr>
  </table>
 </td>
</tr>
